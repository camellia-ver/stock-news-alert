...
> 특정 주식 가격 변동에 따라 해당 주식 관련 뉴스를 자동으로 수집하고, Gemini API로 인사이트를 정리해 전송하는 알림 서비스

---

## 📌 프로젝트 소개

`stock-news-alert`는 사용자가 지정한 주식 종목의 가격 변동률이 임계값을 초과할 경우, 관련 최신 뉴스를 자동으로 수집하고 **Gemini API**를 활용해 하나의 인사이트로 정리한 뒤 알림으로 전송하는 Python 기반 자동화 프로그램입니다.
...
│   └── gemini_summarizer.py # 수집한 뉴스를 Gemini API로 요약/인사이트 생성
├── notifier/
│   ├── __init__.py
│   └── sender.py           # 알림 전송
├── utils
...
4. 수집된 뉴스 기사들을 Gemini API에 전달하여 종목별 하나의 인사이트로 요약/정리
